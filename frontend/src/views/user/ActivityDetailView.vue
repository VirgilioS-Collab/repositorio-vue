...
const { current: activity, loading } = storeToRefs(activityStore);
...
<template>
  <div v-if="activity && !loading" class="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8">
    <header class="pb-6 border-b-2 border-accent">
      ...
      <h1 class="text-4xl font-extrabold text-darkText tracking-tight">{{ activity.title }}</h1>
    </header>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mt-6">
      <div class="md:col-span-2">
        ...
        <p class="text-gray-700 leading-relaxed whitespace-pre-line">
          {{ activity.description || 'El organizador no ha proporcionado una descripción detallada para esta actividad.' }}
        </p>
      </div>
